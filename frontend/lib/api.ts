...
export async function createMeeting(
    ...
    if (!res.ok) {
        ...
        throw new Error(
            err.detail ?? `Failed to create meeting (${res.status})`,
        );
    }
    return res.json();
}
...
export async function updateMeeting(
    ...
    if (!res.ok) {
        const err = await res.json().catch(() => ({}));
        throw new Error(
            err.detail ?? `Failed to update meeting (${res.status})`,
        );
    }
    return res.json();
}
...
export async function createActionItem(
    ...
    if (!res.ok) {
        const err = await res.json().catch(() => ({}));
        throw new Error(
            err.detail ?? `Failed to create action item (${res.status})`,
        );
    }
    return res.json();
}
...
export async function toggleActionItem(
    ...
    const res = await fetch(
        `${API_BASE}/meetings/${meetingId}/action_items/${actionItemId}`,
        {
            method: "PATCH",
            headers: { "Content-Type": "application/json" },
    ...
    if (!res.ok) {
        const err = await res.json().catch(() => ({}));
        throw new Error(
            err.detail ?? `Failed to update action item (${res.status})`,
        );
    }
    return res.json();
}
...
export async function deleteActionItem(
    ...
    if (!res.ok)
        throw new Error(`Failed to delete action item (${res.status})`);
}
...
export async function removeParticipant(
    ...
    const res = await fetch(
        `${API_BASE}/meetings/${meetingId}/participants/${participantId}`,
        {
            method: "DELETE",
        },
    );
    if (!res.ok)
        throw new Error(`Failed to remove participant (${res.status})`);
}
